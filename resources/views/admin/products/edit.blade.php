...
@section('content')
    <div class="flex justify-between items-center mb-4">
        <h1 class="page-title" style="margin-bottom: 0;">Edit Product: {{ $product->name }}</h1>
        ...
        <div style="padding: 1.5rem;">
            <form action="{{ route('admin.products.update', $product) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    @if($product->image)
                        <div class="mb-4">
                            <img src="{{ Storage::url($product->image) }}" alt="{{ $product->name }}" style="width: 100px; border-radius: 0.5rem; object-fit: cover;">
                        </div>
                    @endif
                ...
                <div class="grid-2">
                    <div class="form-group">
                        ...
                        <input type="number" step="0.01" id="purchase_price" name="purchase_price" class="form-control" value="{{ old('purchase_price', $product->purchase_price) }}" required oninput="calculatePrices()">
                        @error('purchase_price') <span class="text-danger">{{ $message }}</span> @enderror
                    </div>

                    <div class="form-group">
                        <label for="stock" class="form-label">Stock Quantity *</label>
                        <input type="number" id="stock" name="stock" class="form-control" value="{{ old('stock', $product->stock) }}" required min="0">
                ...
                <div class="form-group">
                    ...
                    <input type="number" step="0.01" id="discount_percentage" name="discount_percentage" class="form-control" value="{{ old('discount_percentage', $product->discount_percentage) }}" min="0" max="100">
                ...
                <div class="grid-2">
                    <div class="form-group">
                        ...
                        <input type="number" step="0.01" id="cash_margin_percentage" name="cash_margin_percentage" class="form-control" value="{{ old('cash_margin_percentage', $product->cash_margin_percentage) }}" required oninput="calculatePrices()">
                        ...
                        <div class="mt-4 p-3" style="background: var(--bg-color); border-radius: 0.5rem; font-weight: 500;">
                            Calculated Cash Sale Price: RS <span id="calc_cash_price">{{ number_format($product->cash_sale_price, 2) }}</span>
                        </div>
                    </div>

                    <div class="form-group">
                        <label for="emi_margin_percentage" class="form-label">EMI Margin Percentage (%) *</label>
                        <input type="number" step="0.01" id="emi_margin_percentage" name="emi_margin_percentage" class="form-control" value="{{ old('emi_margin_percentage', $product->emi_margin_percentage) }}" required oninput="calculatePrices()">
                        @error('emi_margin_percentage') <span class="text-danger">{{ $message }}</span> @enderror
                        <div class="mt-4 p-3" style="background: var(--bg-color); border-radius: 0.5rem; font-weight: 500;">
                            Calculated EMI Sale Price: RS <span id="calc_emi_price">{{ number_format($product->emi_sale_price, 2) }}</span>
                        </div>
                    </div>
                </div>
